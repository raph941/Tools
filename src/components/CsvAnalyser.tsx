...
import { CSVParsedDataType, useHandleCsv } from "../hooks/useHandleCsv";
import { BarChart } from "./BarChart";

export const CsvAnalyser = () => {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [nonStackColumnNames, setNonStackColumnNames] = useState<string>(
    "Submission ID,Respondent ID,Submitted at,Role,Languages/Stack you are familiar with  (not necessarily an expert at) ,Email (Gitstart mail)"
  );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedFile) {
      try {
        const stackData = await parseCsv(
          emailColumnName || "",
          nonStackColumnNames
        );
